...
```{r benef_map, echo=FALSE}

# DEPARTAMENTOS

# Load data
merged_dpto_benef <- readOGR("merged_dpto_benef.shp", encoding = "UTF-8")

# intervals for distribution
bins_dpto_benef <- c(1000, 7000, 15000, 30000, 50000, 100000, 200000)

# color palettes
pal_dpto_benef <- colorBin(palette = "YlOrBr",
                domain = merged_dpto_benef$cantidad, 
...
labels_dpto_benef <- sprintf(
  ...
  merged_dpto_benef$DPTO_DESC, merged_dpto_benef$cantidad) %>%
...
# Load data
merged_dist_benef <- readOGR("merged_dist_benef.shp", encoding = "UTF-8")

# intervals for distribution
bins_dist_benef <- c(0, 500, 1000, 10000, 20000, 50000, 120000)

# color palettes
pal_dist_benef <- colorBin(palette = "YlGnBu",
...
labels_dist_benef <- sprintf(
  ...
  merged_dist_benef$DIST_DESC, merged_dist_benef$cantidad) %>%
  lapply(htmltools::HTML)
...
```{r cons_map, echo=FALSE}

# DEPARTAMENTOS

# Load data
merged_dpto <- readOGR("merged_dpto.shp", encoding = "UTF-8")
...
# Load data
merged_dist <- readOGR("merged_dist.shp", encoding = "UTF-8")
