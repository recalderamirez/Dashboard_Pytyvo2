...
# Load data
BENEF_DPTO <- read_excel("Beneficiarios_updated.xlsx")

BENEF_DPTO <- as.data.frame(BENEF_DPTO)

BENEF_DPTO$DPTO <- as.factor(BENEF_DPTO$DPTO)
#BENEF_DPTO2 <- read_excel("TODOS_DISTRITOS_GROUPED_DPTOS.xlsx")

# Clean and prepare data
BENEF_DPTO <- BENEF_DPTO %>%
  group_by(DPTO) %>%
  summarise(cantidad = sum(`Cantidad de Beneficiarios`)) %>%
  select(DPTO, cantidad)

#BENEF_DPTO2 <- BENEF_DPTO2 %>%
#  na.omit() %>%
#  group_by(DPTO) %>%
...
deptos_benef <- readOGR("Departamentos_Paraguay.shp", encoding = "UTF-8")
...
merged_dpto_benef <- merge(deptos_benef, BENEF_DPTO, by = "DPTO")


# save shapefile
writeOGR(merged_dpto_benef, ".", "merged_dpto_benef2", 
         driver = "ESRI Shapefile", layer_options = "ENCODING=UTF-8")

testshape <- readOGR("merged_dpto_benef2.shp", encoding = "UTF-8")

merged_dpto_benef2 <- rmapshaper::ms_simplify(testshape, 
                                              keep = 0.05, keep_shapes = TRUE)

writeOGR(merged_dpto_benef2, ".", "merged_dpto_benef2", 
         driver = "ESRI Shapefile", layer_options = "ENCODING=UTF-8")

testtest <- readOGR("merged_dpto_benef2.shp", use_iconv = T, encoding = "UTF-8")
...
# Load data
BENEF_DIST <- read_excel("Beneficiarios_updated.xlsx")
...
#BENEF_DIST <- read_excel("Beneficiarios_Dist.xlsx")


# Load shapefile (Distritos de Paraguay)
distritos_ben <- readOGR("Distritos_Paraguay.shp", 
...
# Save as shapefile
writeOGR(merged_dist_benef, ".", "merged_dist_benef2", 
         driver = "ESRI Shapefile", layer_options = "ENCODING=UTF-8")


testshape <- readOGR("merged_dist_benef2.shp", encoding = "UTF-8")

merged_dist_benef2 <- rmapshaper::ms_simplify(testshape, 
                                              keep = 0.05, keep_shapes = TRUE)

writeOGR(merged_dist_benef2, ".", "merged_dist_benef2", 
         driver = "ESRI Shapefile", layer_options = "ENCODING=UTF-8")

testtest <- readOGR("merged_dist_benef2.shp", use_iconv = T, encoding = "UTF-8")

TEST <- as.data.frame(testshape)
...
# Load shapefile (Departamentos de Paraguay)
deptos <- readOGR("Departamentos_Paraguay.shp", encoding = "UTF-8")
...
# Save as shapefile
writeOGR(merged_dpto, ".", "merged_dpto", 
         driver = "ESRI Shapefile", layer_options = "ENCODING=UTF-8")
...
# Save as shapefile
writeOGR(merged_dist, ".", "merged_dist2", 
         driver = "ESRI Shapefile", layer_options = "ENCODING=UTF-8")

testshape <- readOGR("merged_dist2.shp", encoding = "UTF-8")

merged_dist2 <- rmapshaper::ms_simplify(testshape, 
                                              keep = 0.05, keep_shapes = TRUE)

writeOGR(merged_dist2, ".", "merged_dist2", 
         driver = "ESRI Shapefile", layer_options = "ENCODING=UTF-8")

testtest <- readOGR("merged_dist2.shp", use_iconv = T, encoding = "UTF-8")
...
Comercios <- Comercios %>%
  group_by(RUBRO) %>%
  summarise(cant = sum(CAN_COMERCIOS))


Comercios$RUBRO[which(Comercios$RUBRO == "DESPENSAS")] <- "Supermercados y Despensas"
...
write_excel_csv(Comercios, "Comercios_final.csv")
...
merged_dpto_benef <- readOGR("merged_dpto_benef.shp")

TEST <- as.data.frame(merged_dpto_benef)
...
pal_dpto_benef <- colorBin(palette = "YlOrBr",
                domain = merged_dpto_benef$cantidad, bins = bins_dpto, reverse = F)
